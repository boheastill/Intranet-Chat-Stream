...
*   **零入站公网防火墙 (Cloudflare Tunnel)**：VPS 本地服务仅监听 `127.0.0.1:8666`。外部攻击者及全网扫描器无法从公网直接探测该端口。所有请求均由 VPS 主动出站连接 Cloudflare 隧道进行加密中转。
*   **双模式身份鉴权**：
...
*   应急状态下访问 `http://127.0.0.1:8666/?key=vip`，切换并输入密保密码 `66666666` 登录。
...
BASE_URL = "https://ics.example.com"
...
*作者 [Bohea Still](https://boheastill.com/?lang=zh&r=gh-ics) — 独立开发者,承接自动化、AI 管道、软硬件集成项目。*
